...
export default function TermsPage() {
  return (
    ...
    >
      ...
      <p style={p}>
        When you upload a PDF we keep your original file, and the finished document is produced by stamping the signatures onto its actual pages, so your document&apos;s text and formatting are preserved. We also render each page to an image; that rendering is what you and your signers see on screen while placing fields and signing.
      </p>
      <p style={p}>
        An individual page is reproduced as an image instead when we cannot use the original — you uploaded an image file rather than a PDF, the PDF exceeds 50 MB, it cannot be reopened because it is encrypted or damaged, or the page is rotated. This is decided page by page, and the Certificate of Completion records how many pages came from your original. We do not perform optical character recognition, so a document that had no selectable text to begin with will have none afterwards. Envelopes sent before September 9, 2026 were assembled entirely from page images.
      </p>
