...
function CheckoutSuccessInner() {
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", padding: "40px 20px", textAlign: "center" }}>
        ...
        {envelope.signers.filter((s) => !s.isSelf).map((s) => {
          ...
          return (
            <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 0", borderBottom: "1px solid var(--line)" }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 16, color: "var(--ink)" }}>{s.name}</div>
                <div style={{ fontSize: 16, fontFamily: "'Plus Jakarta Sans', sans-serif", color: "#8A8F98" }}>{s.email}</div>
              </div>
              <span style={{ fontSize: 16, fontFamily: "'Plus Jakarta Sans', sans-serif", color: sentEvent ? "#4E8B5A" : failedEvent ? "#C1440E" : "#9AA0AA" }}>
      ...
      <p style={{ fontSize: 16, color: "#9AA0AA", marginTop: 18, lineHeight: 1.5 }}>
...
export default function CheckoutSuccessPage() {
  return (
    <Suspense fallback={<Centered><h2 style={h2}>Loading…</h2></Centered>}>
