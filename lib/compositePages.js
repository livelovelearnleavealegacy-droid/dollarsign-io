  ...
  return Promise.all(pages.map((page) => new Promise((resolve) => {
    ...
    base.onload = async () => {
      ...
      for (const f of pf) {
        ...
        if (f.kind === "date") {
          ctx.font = `${Math.round(page.w * 0.016)}px 'IBM Plex Mono', monospace`;
          ctx.fillStyle = "#102A43";
          ctx.fillText(f.value, px, py);
        } else if (f.kind === "text") {
          ctx.font = `${Math.round(page.w * 0.018)}px 'Public Sans', sans-serif`;
          ctx.fillStyle = "#102A43";
          ctx.fillText(f.value, px, py);
      ...
      resolve({ id: page.id, url: canvas.toDataURL("image/png") });
    };
    base.src = page.src;
  })));
}
