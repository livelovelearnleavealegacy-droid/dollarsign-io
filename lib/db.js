...
{
  const cols = db.prepare(`PRAGMA table_info(page_files)`).all().map((c) => c.name);
  if (!cols.includes("source_id")) db.exec(`ALTER TABLE page_files ADD COLUMN source_id TEXT`);
  if (!cols.includes("source_page")) db.exec(`ALTER TABLE page_files ADD COLUMN source_page INTEGER`);
}
