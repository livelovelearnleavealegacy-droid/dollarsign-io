...
/* Every envelope, newest first, for the operator view.
   ...
   belongs in the route that knows what the screen needs — and at this
   scale reading a few hundred rows costs nothing. Revisit if the table
   ever gets large enough that "a few hundred" stops being true. */
export function listAllEnvelopes({ limit = 200, status = null } = {}) {
  const rows = status
    ? db.prepare(`SELECT * FROM envelopes WHERE status = ? ORDER BY created_at DESC LIMIT ?`).all(status, limit)
    : db.prepare(`SELECT * FROM envelopes ORDER BY created_at DESC LIMIT ?`).all(limit);
