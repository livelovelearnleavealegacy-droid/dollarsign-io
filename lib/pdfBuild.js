...
function addCertificate(pdfDoc, envelope, fonts, render = {}) {
  ...
  // "paid" is drawn as "sent to signers": it is the moment invitations
  // actually went out, which is what a certificate needs to evidence.
  // DocuSign's equivalent shows a Sent event; ours had the timestamp in
  // the audit log and never drew it. No amount is shown — what the
  // sender paid is nobody else's business.
  const shown = ["created", "paid", "consent", "signed", "completed", "declined", "voided", "expired", "email_sent", "email_bounced", "address_corrected", "invite_resent", "reminder_sent"];
  for (const e of (envelope.auditLog || []).filter((x) => shown.includes(x.type))) {
    newPageIfNeeded(46);
    const heading =
      e.type === "created" ? `Envelope created by ${e.senderName || "sender"}`
      : e.type === "paid" ? "Envelope sent to signers"
      ...
      : e.type === "address_corrected"
        ? `Address corrected — ${e.previousEmail || "previous address"} to ${e.email || "new address"}`
      : e.type === "email_bounced"
        ? `Delivery failed — ${e.email || "recipient"}${e.complaint ? " (marked as spam)" : ""}`
